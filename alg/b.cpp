...
bool dfs(int v, vvi& g, vi& c, int color) {
  c[v] = color;
  fl(next, g[v]) {
    if (c[next] == 0) {
      if (!dfs(next, g, c, (color == 2) ? 1 : 2)) return false;
...
code {
  ...
  fo(i, s.size()) {
    if (islower(s[i])) {
      res += 1;
      animals.push_back(res);
    } else {
      traps.push_back(i - res);
    }

    if (data.empty() || s[i] == data.back()) {
      data.push_back(s[i]);
    } else if (tolower(s[i]) == tolower(data.back())) {
      ids[traps.back()] = animals.back();
      traps.pop_back();
      animals.pop_back();
      data.pop_back();
    } else
