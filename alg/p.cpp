...
void dfs(int v, int* g, vi& cols, int n, int& res, int mx, int c) {
  cols[v] += 1;
  fo(i, n) if ((i != v && cols[i] == c &&
                g[c ? (i * n + v) : (v * n + i)] <= mx))
      dfs(i, g, cols, n, res, mx, c);
  ++res;
}
...
code {
  ...
  for (;;) {
    ...
    res = 0;
    if (n == 1) break;
    dfs(0, &g[0][0], cols, n, res, m, 0);
    dfs(0, &g[0][0], cols, n, res, m, 1);
    if (l >= r) break;
    res == (n << 1) ? r = m : l = m + 1;
  }
  if (res != (n << 1)) l = 0;
