...
<div class="grid grid-cols-10 items-center gap-5 font-bold">
  ...
  <div
    class="col-span-3 flex gap-1 items-center px-2 bg-#445b7b"
    class:disabled
    class:readonly
  ...
  >
    ...
    {:else}
      <input
        {...$$restProps}
        class="bg-transparent text-white font-bold h-16px w-full grow border-none text-right outline-none w-auto min-w-auto border-0px {$$restProps.class}"
        class:invalid
        class:readonly
        {placeholder}
  ...
  <div class="col-span-2 grid whitespace-nowrap items-center">
    {#if units}
      {units}
    {/if}
  </div>
