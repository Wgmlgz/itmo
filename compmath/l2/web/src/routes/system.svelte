<script lang="ts">
  ...
  let fs: F3D[] | undefined = undefined;

  const plotSystem3D = async (fs: F3D[], a: number, b: number, points: number = 400) => {
    const x = Array.from({ length: points }, (_, i) => a + (i * (b - a)) / (points - 1));
    const y = Array.from({ length: points }, (_, i) => a + (i * (b - a)) / (points - 1));

    let z: number[][][] = fs.map((f) => {
      let z: number[][] = [];
    ...
    const data: Partial<PlotData>[] = z.map((z) => ({
      x,
      y,
      z,
      type: 'surface',
  ...
  $: if (input.system) fs = systems.filter(({ id }) => id === input.system)[0].fs;
  $: if (fs && plot) plotSystem3D(fs, -10, 10);

  const solve = () => {
    const { xs, tolerance } = input;
    try {
      if (!fs) return;
      solution = solve_system(fs, xs, tolerance);
...
<Tile>
  <div class="grid grid-cols-2 gap-10">
    ...
    <div class="grid gap-2 h-min">
      ...
      <div class="flex gap-2">
        <NumberInput step={0.001} label="x1" type="number" bind:value={input.xs[0]} />
        <NumberInput step={0.001} label="x2" type="number" bind:value={input.xs[1]} />
